...
function wpgpt_check_version() {
	const req = 'https://wptools.vladtimotei.ro/wpgp-tools/';
	jQuery.get( req, ( wpgpt_last_version ) => {
...
function wpgpt_display_notice() {
	const wpgpt_update_output = $wpgpt_createElement( 'div', { 'class': 'wpgpt-update-notice' } );
	const wpgpt_update_new_version = $wpgpt_createElement( 'strong', {}, `WPGPTools version ${wpgpt_settings.last_version.state} is available! ` );
	const wpgpt_update_current_version = document.createTextNode( `You are using version ${WPGPT_VERSION}. Update now!` );
	const wpgpt_update_br = $wpgpt_createElement( 'br' );
	let wpgpt_update_link, wpgpt_update_instruction;
	if ( 'undefined' === typeof wpgpt_is_userscript ) {
